	...
	<div class="section-even lg:py-10">
		<div class="row bgr_red lg:mb-4">
			<div class="container grid grid-cols-1 lg:grid-cols-8 lg:gap-10 items-center mx-auto p-4">
				<div class="col-span-full lg:col-span-6">
					<p class="gamesSectionHead text-center lg:text-left">{{ msgTranslate?.casino_games || 'Casino Games' }}</p>
				</div>
				<div class="lg:block lg:col-span-2">
					<div class="">
						<NuxtLink to="/casino-games" class="cas-btn">
							{{ msgTranslate?.see_more || 'See More' }} {{ msgTranslate?.casino_games || 'Casino Games' }}
		...
		<div v-else class="px-4 sm:px-6 md:px-0">
			<div class="container mx-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
				<div v-for="game in casinoGames.slice(-12).reverse()" :key="game.id" :class="'item ' + game.id">
					<div class="">
						<div class="show show-first first-content-border">
							...
							<div class='mask'>
								<a :href="regLink" target="_blank">
									<div class="gameDescr">
										<div v-if="casinoGame && game.description && game.description.length > 0"
...
import { fetchGames, games, casinoGames, msgTranslate, regLink, loginLink, loadLang } from '~/composables/globalData';
// Define emit
...
await useAsyncData('casino-games-data', async () => {
	try {
		await loadLang();
